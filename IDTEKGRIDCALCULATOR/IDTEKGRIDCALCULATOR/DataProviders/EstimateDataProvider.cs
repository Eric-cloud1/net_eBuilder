...
namespace IDIGITALTEKGRID
{
	...
	public class EstimateDataProvider
	{
        ...
        public DataTable selectExport(string _id, int _report)
        {
            ...
            for (int k = 0; k < 45; k++)
            {
               dsTable.Columns.Add();
                ...
                IDIGITALTEKGRID.EstimateDatas _estimateDatas = new EstimateDatas();
               ...
               {
                   IDIGITALTEKGRID.EstimateData _value = (IDIGITALTEKGRID.EstimateData)enumerator.Current;
        ...
        public  IDIGITALTEKGRID.EstimateDatas select(string _id, int _report)
		{
          ...
          //  "Discount Favorites Vendors"
            ...
            IDIGITALTEKGRID.EstimateDatas _estimateDatas = new IDIGITALTEKGRID.EstimateDatas();
                ...
                foreach (DataRow row in dsData.Tables[0].Rows)
                {
                    ...
                    decimal _quantityValue = IDIGITALTEKGRID.Operators.Calculate(_expressionID, _JobID);

                    if (_quantityValue  != -999)
                    {
                        IDIGITALTEKGRID.Builder.progressEstimate.Increment(10);
                        IDIGITALTEKGRID.EstimateData _estimateData = new IDIGITALTEKGRID.EstimateData();
                        ...
                        try
                        {
                            _estimateData.Quantity = IDIGITALTEKGRID.Operators.multiplyValues(_quantityValue, Convert.ToDecimal(row["QuantityAdjusted"]));
                        }
                        catch
                        {
                            _estimateData.Quantity = _quantityValue;
                        }
                         _estimateData.QuantityAdjustment = IDIGITALTEKGRID.Operators.quantityAdjustment(_estimateData.PiecePerUnitOrder, _estimateData.Quantity, _estimateData.QuantityMinimum, _estimateData.QuantityRoundUp, _estimateData.QuantityReuse, _estimateData.QuantityWaste);
                         _estimateData.CostAdjusted = IDIGITALTEKGRID.Operators.multiplyValues(_estimateData.CostAdjustment, _estimateData.CostAmt);
                         _estimateData.EstimateCost = IDIGITALTEKGRID.Operators.multiplyValues(_estimateData.QuantityAdjustment, _estimateData.CostAdjusted);
